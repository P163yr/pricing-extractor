...
async function rateGate() {
  ...
  const now = Date.now();
  const wait = Math.max(0, _rlLast + minGap - now);
...
async function llmChat(messages, useJsonMode, attempt, noFast) {
  ...
  if (CFG.fast && !noFast) {
    // NVIDIA NIM / GLM: thinking is ON by default. These turn it off for speed.
    body.chat_template_kwargs = { enable_thinking: false };
    body.include_reasoning = false;
    body.reasoning_effort = "none"; // harmless if the provider ignores it
  }
  let res;
  try {
  ...
  if (!res.ok) {
    ...
    // Provider rejects the reasoning-off flags -> retry once without them.
    if (CFG.fast && !noFast && /chat_template_kwargs|enable_thinking|include_reasoning|reasoning_effort|unexpected|unknown|unsupported|invalid|400/i.test(errText)) {
...
<div class="grid">
 <div class="panel"><h2>Configuration</h2>
  ...
  <div class="row2"><div><label>Rate limit (calls/min, 0=off)</label><input type="text" id="rpm" value="40"/></div><div><label>&nbsp;</label><div class="hint" style="margin-top:9px">Set to your provider's limit. NVIDIA GLM ≈ 40.</div></div></div>
...
async function start(){
 ...
 const body={apiKey:$("apiKey").value.trim(),model:$("model").value,endpoint:$("endpoint").value.trim(),
  concurrency:$("concurrency").value,timeout:$("timeout").value,maxSteps:$("maxSteps").value,rpm:$("rpm").value,
