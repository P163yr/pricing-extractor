...
/* ---------------- LLM call (DeepSeek / Cerebras / any OpenAI-compatible) ---------------- */
async function llmChat(messages, useJsonMode) {
...
async function agentDecide(siteUrl, st, history, step, force) {
  ...
  for (let attempt = 1; attempt <= ATTEMPTS; attempt++) {
    try {
      ...
      // API error / rate-limit blip — wait and retry
    }
    if (attempt < ATTEMPTS) await sleep(800 * attempt); // 0.8s, then 1.6s backoff
  }
  return { action: "done", thought: "parse failed after retries",
    result: { pricing_model: "unknown", price_point: null, currency: "", price_display: "", has_free_tier: false, has_annual_plan: false, notes: "agent parse error" } };
}
...
<div class="grid">
 <div class="panel"><h2>Configuration</h2>
  ...
  <div class="row2"><div><label>Provider preset</label><select id="preset" onchange="applyPreset()">
    ...
    <option value="llama-4-scout-17b-16e-instruct|https://api.cerebras.ai/v1/chat/completions">Cerebras Llama 4 Scout 17B (better agent)</option>
    <option value="llama-3.3-70b|https://api.cerebras.ai/v1/chat/completions">Cerebras Llama 3.3 70B (better agent)</option>
    <option value="llama3.1-8b|https://api.cerebras.ai/v1/chat/completions">Cerebras Llama 3.1 8B (weakest — may struggle)</option>
