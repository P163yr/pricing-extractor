...
async function llmChat(messages, useJsonMode, attempt) {
  attempt = attempt || 1;
  await rateGate();
  ...
  try {
    ...
    if (attempt <= 4) { await sleep(1500 * attempt); return llmChat(messages, useJsonMode, attempt + 1); }
    throw netErr;
  }
  if (!res.ok) {
    ...
    if ((res.status === 429 || res.status >= 500) && attempt <= 5) {
      ...
      await sleep(backoff);
      return llmChat(messages, useJsonMode, attempt + 1);
    }
    ...
    if (useJsonMode && /response_format|json|400|unsupported|invalid/i.test(errText)) {
      return llmChat(messages, false, attempt);
    }
